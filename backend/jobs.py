...
lives in backend/agents/relationship/*, backend/integrations/*, etc., and is
imported by BOTH the local path (routes call it directly) and the Modal path
(modal_jobs.py imports the same functions). Single source of truth.
"""
...
# --------------------------------------------------------------------------- #
# Async Job model for request/response background work.
#
# Some jobs (e.g. the LinkedIn conversation import) are request/response: the
# frontend needs the result back. So instead of blocking the HTTP handler, we:
#   1. create a queued Job row,
#   2. dispatch the work (Modal when USE_MODAL, else a local thread),
#   3. hand the job id back immediately,
# and the worker writes the serialized result onto the Job row. The frontend
# polls until status == done and reads result_json.
# --------------------------------------------------------------------------- #
