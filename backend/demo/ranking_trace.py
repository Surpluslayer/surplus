...
def _latest_signal(db, contact_id: int):
    """Most recent detected-signal interaction on this contact (real
    source_type taxonomy: 'activity_update')."""
    return db.execute(
...
def _all_interactions(db, contact_id: int) -> list:
...
def _signal_relevance(signal) -> RankingFactor:
    if signal is None:
...
def _historical_behavior(db, user, signal) -> RankingFactor:
...
def compute_trace(db, user, contact) -> RankingTrace:
    signal = _latest_signal(db, contact.id)
    interactions = _all_interactions(db, contact.id)

    factors = [
        _signal_relevance(signal),
        _practice_fit(user, signal),
        _relationship_strength(interactions),
        _relationship_recency(interactions),
        _relationship_trajectory(interactions),
        _historical_behavior(db, user, signal),
    ]
    score = sum(f.value * FACTOR_WEIGHTS[f.name] for f in factors)
    return RankingTrace(contact_id=contact.id, contact_name=contact.name or "",
                        factors=factors, opportunity_score=score)


def rank_opportunities(db, user, contacts: list) -> list[RankingTrace]:
    traces = [compute_trace(db, user, c) for c in contacts]
