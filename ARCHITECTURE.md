...
- `relationships.py` — event-native **read model** (timeline, contact_summary, list_contacts) + `import_conversation_contacts()`. `contact_summary` carries a **capture-provenance rollup** (`origin` = captured / sourced / conversation, `met_in_person`, `met_event`/`met_on`, `capture_method`, `capture_note`) — `captured_at` is only ever written by the in-person capture flow, so it is the honest "did the host actually meet them" discriminator (`met_at` alone is just the event we know them from). The agent roster + `_context_brief` consume it: met-in-person becomes a safe fact (with the capture note), sourced-never-met becomes an explicit do-not-imply-having-met guard. `link_contact`/`prospect_company` never store placeholder companies ('Unknown') — the employer is parsed from the capture role string ("Engineer @ AutoComplete") when needed; `/admin/backfill-contact-companies` repairs existing rows. *(distinct from routes/relationships.py)*
- `updates_engine.py` — **the updates orchestrator**: `run_sweep` (Bright Data async trigger AND the sync web pass, additive — `UPDATES_WEB_ADDITIVE=0` restores fallback-only), `scrape_contact` (one-contact immediate check, used on ⭐-star and by the agent's pre-draft fresh-check), `due_contacts` (vip=daily/others=weekly tiering), `apply_profile`/`apply_posts` (diff + baseline-first), `autodraft` (drafts only `_DRAFTWORTHY_KINDS`).
- `updates_scheduler.py` — in-process daemon that claims+runs the sweep hourly (shared `scheduler_claims` row dedups with Modal).
- `updates_watch.py` — the account-safe **web pass**, three legs merged into ONE extraction + identity gate: Google News RSS (keyless; the "what people find when they google them" leg — press/raise coverage), Exa web (semantic catch-all), and Exa scoped to x.com (sharpened by an `x_handle` ContactFact, resolved once via Exa and cached, misses too). Per-leg kill switches `UPDATES_NEWS_ENABLED` / `UPDATES_X_ENABLED`. `relationship_watch.py` — Unipile CRM poller; `_emit()` writes every `activity_update` **and fires autodraft** (single choke point).
...
3. Two additive source passes per due contact: **Bright Data** scrapes their public LinkedIn profile/posts on its own infra → delivers to `/webhooks/brightdata`; the **sync web pass** (`updates_watch`: Google News RSS + Exa web + Exa-x.com) emits immediately. The relationship agent also fires `scrape_contact` for anyone it is about to draft for whose `watched_at` is missing/stale (`AGENT_FRESH_CHECK_DAYS`/`_MAX`), so drafts ground in fresh reality. Posts use `only_authored_posts=true` (their own posts only, not the activity feed) — keeps the signal clean and slashes credit burn (a non-poster = 0 records).
