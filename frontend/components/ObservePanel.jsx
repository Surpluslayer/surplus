...
import {
  ...
  CheckCircle2, XCircle, Circle, Layers, FlaskConical, Beaker,
} from "lucide-react";
...
// ── Ablate action: full system vs without-relationship, for one contact ──
function AblateButton({ contactId }) {
  const [result, setResult] = useState(null);
  const [loading, setLoading] = useState(false);
  const run = () => {
    setLoading(true);
    getJSON(`/api/observe/ablate/${contactId}`, { remove_group: "relationship" })
      .then(setResult).catch(() => {}).finally(() => setLoading(false));
  };
  return (
    <div style={{ marginTop: 10 }}>
      <button onClick={run} disabled={loading} style={{
        fontSize: 11.5, fontWeight: 700, color: C.accent, background: C.accentBg,
        border: "none", borderRadius: 6, padding: "5px 10px", cursor: "pointer",
      }}>
        <Beaker size={11} style={{ verticalAlign: -1, marginRight: 5 }} />
        {loading ? "Ablating relationship…" : "Ablate relationship"}
      </button>
      {result && (
        <div style={{ marginTop: 8, fontFamily: MONO, fontSize: 11.5, color: C.ink }}>
          Full system: rank #{result.full_system.rank} (score {result.full_system.score})<br />
          Without relationship: rank #{result.without_group.rank} (score {result.without_group.score})<br />
          <span style={{ color: result.rank_delta > 0 ? C.good : C.faint }}>
            Relationship contribution: {result.rank_delta > 0 ? "+" : ""}{result.rank_delta} ranking positions
          </span>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ border: `1px solid ${C.line}`, borderRadius: 12, background: C.card,
      ...
      <TraceSection title="Ranking" icon={TrendingUp}
        fetchTrace={() => getJSON(`/api/observe/trace/opportunity/${contactId}`)}
        extra={<AblateButton contactId={contactId} />} />
